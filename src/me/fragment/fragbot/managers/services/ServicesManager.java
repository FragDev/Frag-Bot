package me.fragment.fragbot.managers.services;
...
public class ServicesManager {
	...
	public ServicesManager() {
		...
		textChannel.getHistory().retrievePast(100).queue(messages -> {
			if (messages
					.stream().filter(
							message -> message.getAuthor().getId().equals(FragBot.getJda().getSelfUser().getId()) && message.getActionRows().stream()
									.allMatch(actionRow -> actionRow.getButtons().stream()
											.allMatch(button -> button.getId().equalsIgnoreCase("service-plugin") || button.getId().equalsIgnoreCase("service-discordbot"))))
					.count() == 0) {
				this.textChannel.sendMessage(new EmbedBuilder().setColor(new Color(47, 49, 54))
						.setAuthor(this.textChannel.getGuild().getName(), null, this.textChannel.getGuild().getIconUrl())
